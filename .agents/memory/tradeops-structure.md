...
## drizzle-kit push hangs/errors non-interactively on fresh DB
`drizzle-kit push`/`push-force` invokes an ink-based interactive prompt (`promptNamedWithSchemasConflict`) even on a brand-new empty database with zero name collisions; piping newlines or running under `script` for a fake TTY still hangs or errors with "Interactive prompts require a TTY terminal". For a from-scratch dev DB, skip drizzle-kit entirely and apply hand-written idempotent `CREATE TABLE IF NOT EXISTS` SQL (matching each `pgTable` schema file's columns/constraints exactly) via `psql "$DATABASE_URL"` instead.
**Why:** re-importing this project onto a fresh Replit-provisioned Postgres (empty except for connect-pg-simple's `sessions` table) needed `users`/`weeks`/`trades` created before the app's own idempotent startup-migration step (which only ALTERs/creates auxiliary tables, assuming the core Drizzle tables already exist) could succeed.
